...
class AnkiTabKanjiModelWidget extends ConsumerStatefulWidget {
  const AnkiTabKanjiModelWidget({super.key, required this.sortedModels});

  final List<ModelInfo> sortedModels;

  @override
  ConsumerState<AnkiTabKanjiModelWidget> createState() =>
      _AnkiTabKanjiModelState();
}

class _AnkiTabKanjiModelState extends ConsumerState<AnkiTabKanjiModelWidget> {
          ...
          Expanded(
            child: Scrollbar(
              child: ListView.separated(
                itemCount: widget.sortedModels.length,
                separatorBuilder: (ctx, idx) => Divider(),
                itemBuilder: (ctx, idx) {
                  final model = widget.sortedModels[idx];
                    ...
                    : () async {
                        ...
                        try {
                          ...
                          for (var model in selectedModels) {
                            ...
                            while (hasMore) {
                              var batch = await AnkiService().getCardsForModel(
                                model.modelId,
                                model.selectedField!,
                                offset: offset,
                                limit: batchSize,
                              ...
                              for (var card in batch) {
                                kanjiBankData[card.kanji] = KanjiBankEntry(
