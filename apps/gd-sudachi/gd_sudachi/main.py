...
try:
    ...
    from .kanji_bank import load_kanji_bank, is_kanji
            ...
            for _html_i in range(len(html_text)):
                ...
                if accumulator_sudachi >= len(character):
                    if bank_data:
                        ...
                        log("\t\t\tBank Vocab Found:", updated_html[html_start_rel_sudachi+updated_html_start_shift:tmp_accumulator_html+updated_html_end_shift])
                        opening_tag = f'<span class="{pos} {bank_data.get("source")} {bank_data.get("type")} stage-{bank_data.get("level").lower()}">'
                        closing_tag = f"</span>"
                        log("\t\t\t", updated_html)
                        updated_html = updated_html[:html_start_rel_sudachi+updated_html_start_shift] + f"{opening_tag}{character}{closing_tag}" + updated_html[tmp_accumulator_html+updated_html_end_shift:]
                        ...
                        for i, char in enumerate(character):
                            ...
                            # TODO: 紹介所 - 所 cab be saved as both kanji and vocabulary but only one is in the kanji bank
                                ...
                                if individual_bank_data:
                                    tmp_accumulator_html = accumulator_html + i - (len(character) - 2)
                                    tmp_range = updated_html[html_start_rel_sudachi+updated_html_start_shift+i:tmp_accumulator_html+updated_html_end_shift]
                                    log(f"\t\t\tBank Kanji Found: {char}/{tmp_range}")
                                    opening_tag = f'<span class="{pos} {individual_bank_data.get("source")} kanji stage-{individual_bank_data.get("level").lower()}">'
                                    closing_tag = f"</span>"
                                    log("\t\t\t", updated_html)
                                    updated_html = updated_html[:html_start_rel_sudachi+updated_html_start_shift+i] + f"{opening_tag}{char}{closing_tag}" + updated_html[tmp_accumulator_html+updated_html_end_shift:]
                                    updated_html_start_shift += len(opening_tag) + len(closing_tag)
                                    updated_html_end_shift += len(closing_tag)  + len(opening_tag)
                                    log("\t\t\t", updated_html)
                                    known_kanji += 1
                                    if len(tmp_range) != 1:
                                        panic(f"Only 1 character not in range: {tmp_range}")
