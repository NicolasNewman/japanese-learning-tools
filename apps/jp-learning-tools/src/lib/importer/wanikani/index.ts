...
export default class WaniKaniImporter extends Importer<WaniKaniMetadata> {
  ...
  async load(): Promise<WaniKaniKanjiBankData> {
    ...
    let assignmentUrl:
      | string
      | null = `https://api.wanikani.com/v2/assignments?started=true&subject_types=kanji,vocabulary`;
    ...
    while (assignmentUrl) {
      ...
      if (!assignmentsResp.ok) {
        throw new Error(
          `Failed to fetch assignments from WaniKani API: ${assignmentsResp.status} ${assignmentsResp.statusText}`,
        );
      }
      ...
      if (isAssignmentCollection(assignments)) {
        ...
        const assignmentToSrsStage = assignments.data.reduce(
          (prev, { data }) => {
        ...
        if (!subjectsResp.ok) {
          throw new Error(
            `Failed to fetch subjects from WaniKani API: ${subjectsResp.status} ${subjectsResp.statusText}`,
          );
        }
        const subjects = await subjectsResp.json();
        if (isSubjectCollection(subjects)) {
          kanjiBankData = subjects.data.reduce((prev, { data, object, id }) => {
            // For single character vocabulary, default to the kanji entry if it exists
            if (
              object === "kanji" ||
              (object === "vocabulary" && !prev[data.characters])
            ) {
              ...
              if (object === "kanji") {
                prev[data.characters].metadata.kanjiData = {
                  onyomiReadings: data.readings
                    .filter((r) => r.type === "onyomi")
                    .map((r) => r.reading),
                  kunyomiReadings: data.readings
                    .filter((r) => r.type === "kunyomi")
                    .map((r) => r.reading),
                  nanoriReadings: data.readings
