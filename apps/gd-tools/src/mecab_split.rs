...
impl MecabParams {
    ...
    fn assign(&mut self, key: &str, value: &str) -> Result<()> {
        if key == "word" {
            ...
            return Err(GdToolsError::InvalidArgument(format!("Unknown argument name: {}", key)));
        }
        Ok(())
    }
}
...
fn lookup_words(params: MecabParams) -> Result<()> {
    ...
    if !params.dic_dir.is_dir() {
        return Err(GdToolsError::ServiceUnavailable("Couldn't find dictionary directory.".into()));
    }
    ...
    let mut child = cmd
        ...
        .spawn()
        .map_err(|e| GdToolsError::ServiceUnavailable(format!("Failed to spawn MeCab process: {}", e)))?;

    // Write the input text to MeCab's stdin
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(gd_sentence.as_bytes())
            .map_err(|e| GdToolsError::ServiceUnavailable(format!("Failed to write to MeCab process: {}", e)))?;
        // Closing stdin is important to avoid deadlocks
    }

    // Get output from MeCab
    let output = child.wait_with_output()
        .map_err(|e| GdToolsError::ServiceUnavailable(format!("Failed to get MeCab output: {}", e)))?;
    
    if !output.status.success() {
        return Err(GdToolsError::ServiceUnavailable("MeCab process failed".into()));
    }
    
    // Convert output to string
    let result = String::from_utf8(output.stdout)
        .map_err(|e| GdToolsError::ServiceUnavailable(format!("Invalid UTF-8 output from MeCab: {}", e)))?;
    
    if result.is_empty() {
        return Err(GdToolsError::ServiceUnavailable("Failed to parse text with MeCab.".into()));
    }
